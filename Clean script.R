...
########################################
# CODE FOR COSTA RICA ANALYSIS

setwd("D://")

folddirs<- list.dirs()
folddirs<-folddirs[!grepl("RECYCLE", folddirs, ignore.case = TRUE)]
folddirs<-folddirs[!grepl("System", folddirs, ignore.case = TRUE)]
folddirs<-folddirs[!grepl("Seagate", folddirs, ignore.case = TRUE)]
folddirs<-folddirs[!grepl("INCOMPLETE", folddirs, ignore.case = TRUE)]
folddirs<-folddirs[!grepl("FAILED", folddirs, ignore.case = TRUE)]
folddirs<-folddirs[!grepl("LowQual", folddirs, ignore.case = TRUE)]

#CHECK THAT ONLY VALID FOLDERS IN FOLDDIRS
print(folddirs)
##

allsites<- list()
start_times <- seq(0,720, by=1)

for (i in 2:length(folddirs)) {
  ...
  for (j in 2:11) {
    ...
    for (k in 1:720) {
      ...
      wave<-readWave(filenames[j], from = start_times[k], to = start_times[k] + 1, units = 'minutes')
      
      Result<-AcouIndexAlpha(wave, stereo=FALSE, min_freq = 0, max_freq = 20000, anthro_min = 0, anthro_max = 290, bio_min=290,
                             
                             bio_max=16000, wl=512, j=5, AcouOccupancy=TRUE, Bioac=TRUE, Hf=TRUE, Ht=TRUE, H=TRUE, ACI=TRUE, AEI_villa=TRUE, M=TRUE, NDSI=TRUE,
  ...
  allsites[[i]] <- TableTotal
  
}

names(allsites) <- folddirs
